            ...
            for u in usersSockets:
                if u == receiver:
                    ...
                    if currSocket is not None:
                        currSocket.send(text.encode('utf-8'))
                        break
            ...
            for groupName in chatGroupUsers:
                if groupName == gpReceiver:
                    for memberUsername in chatGroupUsers[groupName]:
                        memberSocket = usersSockets[memberUsername]
                        memberSocket.send(text.encode('utf-8'))
                    break
        elif requestSp[0] == 'createGP' and len(requestSp) > 2:
            groupName = requestSp[1]
            groupMembers = requestSp[2]
            chatGroupUsers[groupName] = groupMembers
            print('successfully created a group')
...
def handle_client(client_socket, client_address):
    ...
    usersSockets[username] = client_socket
    print('usersockets')
    print(usersSockets)
    while True:
